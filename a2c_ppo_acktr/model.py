...
class Policy(nn.Module):
    ...
    def evaluate_actions(self, inputs, rnn_hxs, masks, action, eval_prev_mean=None):
        ...
        for i in range(inputs.size()[0]):
            value, actor_features, _ = self.base(inputs[i,:,:], rnn_hxs, masks[i,:,:])
        ...
        reg = torch.stack(reg_policy).mean()

        return v, action_log_probs, dist_entropy, reg, rnn_hxs, eval_prev_mean
